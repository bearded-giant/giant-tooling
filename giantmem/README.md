...
- `giantmem mcp serve` — six MCP tools so Claude can self-discover state: `search_archive`, `list_sessions`, `get_session_summary`, `recent_writes`, `feature_status`, `workspace_tree`. All registered with read-only / idempotent / closed-world hints. `search_archive` mirrors the CLI: auto-quotes punctuation queries (`hub-and-spoke`), accepts `tool_filter` / `ext_filter` / `include_read` for per-line decoded session matches (role + tool name + file path + excerpt), and falls back to file-level results when no session filters are set.
