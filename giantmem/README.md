...
- `giantmem project list|delete` — list every live-index project (doc/artifact/archive counts, gone flag for missing worktrees) and remove one with a y/N prompt. Delete cascades live_docs (+fts), artifacts, embeddings, and access rows; archives.db documents survive unless `--purge-archive` (matches `canonical_project` too, so archive-only session projects are deletable). The GUI mirrors this via right-click on any project list — tree sidebar, sessions project facet, activity list — with an archive-purge toggle in the confirm modal.
