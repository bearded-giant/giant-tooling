...
| Memory + CLI | `~/dev/giant-tooling/giantmem/` (this repo) | `giantmem find`, `giantmem session`, `giantmem feature`, `giantmem db`, `giantmem mcp` |
...
- `giantmem session list|show|find|resume|export|diff|prune-orphans` — find, resume, export, and diff Claude sessions; `resume` chdirs to the recorded cwd (falling back to the `cd` matcher) and exec's `claude --resume <uuid>`. `prune-orphans` lists (and with `--delete` removes) `~/.claude/projects` dirs whose every recorded cwd is gone — keeps any dir with a live cwd, never touches dirs with no recorded cwd.
- `giantmem feature new|start|pause|complete|reopen|list|archive` — feature lifecycle. The five state verbs are thin wrappers over `workspace/scripts/feature.py` (same engine the `/new-feature` family of slash commands calls); `list` reads `features.json` natively; `archive` verifies a feature's files are in live.db, removes its dir, and flips status to archived.
- `giantmem db ingest` — primarily ingests Claude session JSONLs into `archives.db`. The legacy snapshot-workspace pass (`workspace-md`/`domain-json`) is still registered but no longer fed by archiving; live.db is now the authoritative store for `.giantmem/` content.
- `giantmem db index init|migrate|sessions|live|backfill` — bootstrap, consolidate `-wt` projects, backfill cwd, full live rescan. `backfill` walks every `.giantmem/` under `$GIANTMEM_DEV_ROOTS` and upserts every non-empty file (any extension, max 5MB) into `live.db.live_docs`; the daemon also runs this once at startup. Schema is versioned via `PRAGMA user_version`; migrations run automatically on `db.Open`.
- `giantmem archive list|open|dedup|stale` — read-only browser of pre-existing cold snapshot dirs under `~/giantmem_archive/{project}/{ts}/`. Archiving itself moved to the nouns: `feature archive` (per-feature) and `workspace archive` (full wipe + reinit); both verify live.db capture before deleting — no filesystem snapshot is taken. `archive run`/`archive feature` remain as deprecated aliases.
- `giantmem project list|delete` — list every live-index project (doc/artifact/archive counts, gone flag for missing worktrees) and remove one with a y/N prompt. Delete cascades live_docs (+fts), artifacts, embeddings, access rows, and active_sessions; archives.db documents survive unless `--purge-archive` (matches `canonical_project` too, so archive-only session projects are deletable). The GUI mirrors this via right-click on any project list — tree sidebar, sessions project facet, activity list — with an archive-purge toggle in the confirm modal.
...
- `giantmem workspace status|init|migrate|note|discover|complete|sync|archive|gitlog|bootstrap` — workspace lifecycle. Feature verbs moved to `giantmem feature` (old `workspace *-feature` forms remain as deprecated aliases).
...
- `giantmem status`, `giantmem db tail`, `giantmem capture`, `giantmem timeline`, `giantmem plan list`, `giantmem db backup push` — daily-polish utilities (statusline JSON, live.db tail-f, quick brain-dump capture, ASCII activity timeline, cross-workspace plan aggregation, archives.db git-backed snapshot).
...
- `giantmem db watch start|stop|status|run|install` — fsnotify daemon, debounced 2s per workspace, runs `artifact reindex` on `.giantmem/**` edits. PID at `~/.cache/giantmem/giantmem-watch.pid`. `install` writes a macOS LaunchAgent.
