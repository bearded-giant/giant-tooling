...
- `giantmem mcp serve` — six MCP tools so Claude can self-discover state: `search_archive`, `list_sessions`, `get_session_summary`, `recent_writes`, `feature_status`, `workspace_tree`. All registered with read-only / idempotent / closed-world hints.
