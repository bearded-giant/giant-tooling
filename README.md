...
Go CLI that unifies search, sessions, worktree, workspace, and ingest under one binary (`giantmem`). Replaced the prior bash + python `giantmem-archive/` scripts. Key surfaces: `giantmem find` (FTS5 across live + Claude session transcripts, ranked and merged); `giantmem feature` (lifecycle + archive); `giantmem db index backfill` (filesystem walker that fills live.db from every `.giantmem/`); `giantmem session list|find|resume|export|diff`; `giantmem worktree`, `giantmem workspace`, and `giantmem doctor [--fix]`; plus `giantmem mcp serve` exposing read-only tools so Claude can self-discover state. Archiving (`giantmem feature archive`, `giantmem workspace archive`) is a live.db-verified delete — no filesystem snapshot, no DB feed; `giantmem archive` only browses pre-existing snapshot dirs.
